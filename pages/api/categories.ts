...
export async function getCategories(): Promise<Array<Categories>> {
  const categories = await fetch(`${process.env.NEXT_PUBLIC_HYPEBOLD_API}/ad/category`);
...
export async function getCategoryQueriesByUrl(url: String): Promise<Array<Categories>> {
  const category = await fetch(`${process.env.NEXT_PUBLIC_HYPEBOLD_API}/ad/queries?category=${url}`);
...
export async function getCategoryDataByUrl(url: String): Promise<Array<Categories>> {
  const category = await fetch(`${process.env.NEXT_PUBLIC_HYPEBOLD_API}/ad/category/${url}`);
  const data = await category.json();
  if (category) return await data.data;
  else return null;
}
